...
3. **Percentile**: Where the current provider-sourced PE ranks among all monthly reported-EPS PEs in the past 10 years
4. **Status**: ≤25% = Low (undervalued) · 25-75% = Neutral · ≥75% = High (overvalued)

Company cards and PE percentile both use provider-sourced current `peTtm`; `peTtmComputed` is retained only as the reported-EPS reference value.
...
→ peTtm = provider-sourced current PE → percentile ranking
